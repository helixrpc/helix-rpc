...
pub async fn sniff_protocol(stream: &TcpStream) -> std::io::Result<Protocol> {
    ...
    while bytes_peeked < 6 && start.elapsed() < Duration::from_millis(100) {
        let peek_fut = stream.peek(&mut buf);
        if let Ok(Ok(n)) = tokio::time::timeout(Duration::from_millis(10), peek_fut).await {
            bytes_peeked = n;
            ...
            if bytes_peeked >= 2 && (buf[0] != 0x00 || buf[1] != 0x00) {
                break;
            }
        } else {
            break;
